...
- A new argument `date` to `Altimetry.predefined_temporal_filter_atl06sr`, which can be used to pass the capture date of the scene. Previously, the date was read from metadata, but that only works for WorldView right now.
