...
Repository-size work. The committed example reports and notebooks are build outputs that never delta-compress, so every regenerated copy is a permanent new blob — 846 MB of PDF and 756 MB of notebook across all history, against a ~275 MB checkout. Reports now cap embedded figure resolution at what the page can actually show, the committed notebooks are downsampled in place, and pre-commit hooks enforce both so the rules do not depend on anyone remembering them. No new runtime dependencies and no entry-point changes.

### Added
- **Figures embedded in the PDF report are capped at 200 effective dpi** (`asp_plot.report.FIGURE_MAX_DPI`, `asp_report --figure-max-dpi`, `0` to disable). Figures are plotted at 220 dpi and then scaled down to fit the page, so the resolution that lands in the PDF is set by the *placed* size: a 24-inch-wide multi-panel figure squeezed into the 186 mm usable width of a Letter page arrives at ~400 dpi. Measuring the committed Atlanta MVS report showed 21.3 of its 21.4 MB is image data, 13.6 MB of it at 300–500 effective dpi. `compile_report()` now downsamples each figure to the ceiling at its placed width using PIL (already a dependency; fpdf2's `image()` accepts a `PIL.Image`, so there are no temp files). Resolution-only downsampling was chosen over a Ghostscript-style pass — `gs -dPDFSETTINGS=/ebook` reached 1.9 MB on the same report, but by re-encoding plots as JPEG, which rings around axis lines and tick labels. **The six regenerable example reports are regenerated under the cap: 76.3 → 56.4 MB**, each with `--reuse-selections` so every page stays figure-for-figure comparable with the version it replaces.
- **`tools/shrink_notebook_outputs.py`**, which downsamples the base64 PNGs embedded in committed notebooks without re-executing them. Notebook outputs are ~99.9% of notebook bytes — the largest example is 28 MB of which 21 KB is source — and re-running at a lower plot dpi would need the full example datasets. Caps at 1600 px (2× the ~800 px the docs theme lays an output out at, so HiDPI stays sharp); **the 13 committed notebooks go 109.1 → 71.5 MB**, with cell source, execution counts, cell and notebook metadata, and every non-PNG output verified byte-identical. `--colors 256` quantizes as well and reaches ~29 MB, but these figures carry 6,000–14,000 unique colors once several colormaps and a basemap share a page, so it is off by default.
- **`tools/check_report_figure_dpi.py`** and three pre-commit hooks that make the artifact rules mechanical rather than documented. `shrink-notebook-outputs` auto-fixes staged notebooks the way `black` fixes source; `check-report-figure-dpi` fails on a staged report carrying figures above the cap (catching one generated before the cap existed, with `--figure-max-dpi 0`, or copied from an older checkout — all indistinguishable in a viewer); `check-added-large-files` is a 25 MB backstop. Both local hooks declare `pillow`/`pypdf` through `additional_dependencies`, so pre-commit builds isolated venvs and neither becomes an asp_plot dependency.
- **A "Committed Artifacts" section in AGENTS.md** recording why these files are committed at all, that every commit touching them is permanent, and the blobless sparse-clone recipe (~86 MB instead of ~1 GB) for anyone asking why cloning this repository is so expensive.
