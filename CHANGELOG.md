...
### Fixed
- **Report robustness for N-scene and covariance-free sensors** ([#155](https://github.com/uw-cryo/asp_plot/pull/155)). The stereo-geometry report section emits one PDF section per saved figure (N-scene runs produce an overview plus per-pair figures); long DEM-summary values (e.g. three joined acquisition dates) wrap inside the title-page table instead of overflowing the page; and the satellite position/orientation covariance panels annotate "not provided" instead of crashing when a sensor carries no ephemeris/attitude covariance (as DIMAP does not).
