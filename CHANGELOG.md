...
### Added
- **pixi is supported as an alternative to conda for development environments, with a committed lockfile** ([#184](https://github.com/uw-cryo/asp_plot/issues/184)). `pixi.toml` declares the same environment `environment.yml` does — the runtime dependencies as conda-forge packages, plus the `dev` and `docs` extras, plus the package itself installed editable — and `pixi.lock` pins every resolved package for `linux-64`, `osx-arm64`, and `osx-64`, so an environment cannot drift between contributors, between machines, or between a contributor and CI. There is no environment to create or activate: any `pixi run` installs from the lockfile first if the environment is missing. Tasks wrap the common commands (`pixi run setup` for the pre-commit hooks, `test`, `lint`, `docs`, `docs-build`, the last two staging the notebooks, figures, and reports into `docs/` the way ReadTheDocs' `pre_build` job does). Only the editable `./` install resolves from PyPI — GDAL and the rest of the geospatial stack come from conda-forge, as with conda. CI now runs the suite twice, once under each environment manager, with `locked: true` on the pixi job so a `pixi.toml` edit that wasn't followed by `pixi install` fails the build. Nothing about the released package changes: `pyproject.toml` remains the source of truth for version, runtime dependencies, and entry points, and `environment.yml` and the conda workflow are untouched.
