...
- **The Atlanta scene-combination benchmark: `notebooks/WorldView/worldview_spacenet_atlanta_benchmark.ipynb` and `run_benchmark.sh`** (issue [#169](https://github.com/uw-cryo/asp_plot/issues/169)). The systematic version of the MVS notebook's comparison: all ten pairs among the five same-pass scenes (5–32° convergence), three `dem_mosaic` flows (the three pairs sharing the reference, all ten, and only the six above 15°) and four multi-view runs (3, 3-wide, 4 and 5 scenes), seventeen DEMs scored with `DEMBenchmark` on one ICESat-2 sample. The script is idempotent and took 11 h and ~100 GB on a laptop. What it found: single-pair NMAD falls monotonically with convergence angle, 1.5–1.65 m at 5° to 0.62 m at 32°, and IntersectionErr says nothing about it; adding scenes to a joint triangulation barely moves NMAD (0.72 → 0.71 → 0.66 → 0.68 m along the nested chain) but does remove bias; and pairwise + mosaic beats multi-view only when the weak pairs are left out — every pair merged scores 1.00 m against MVS 5's 0.68 m, the six pairs above 15° merged score 0.64 m, the best DEM after the two widest pairs. The mechanism is ASP's multi-view design: every scene is matched to the reference and never to each other, so MVS 5 referenced on the middle scene is built from 5.1°, 10.5°, 16.3° and 21.8° pairs while the widest pairs in the data set never enter it. The notebook's docs card and toctree entry are added.
