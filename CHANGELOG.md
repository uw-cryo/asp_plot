...
### Fixed
- **`glob_file()` sorts its matches, so a directory holding two ASP runs no longer resolves to an arbitrary one** (raised in issue [#6](https://github.com/uw-cryo/asp_plot/issues/6)). `glob.glob()` returns filesystem order, and `glob_file()` took `files[0]` from it, silently. That is invisible for the usual one-run directory, but the two-stage `bundle_adjust` → `pc_align` → `bundle_adjust` workflow leaves two generations of diagnostics side by side (`ba-*` and `ba_pc_align-*`), and `ReadBundleAdjustFiles.get_csv_paths()` globs `*-initial_residuals_pointmap.csv` and `*-final_residuals_pointmap.csv` as two independent lookups — so the pair could come from *different* generations, and the residual plots would then compare one run's initial residuals against another run's final ones. Sorting makes the pick deterministic and, because both patterns differ only in their suffix, makes the initial/final pair come from the same run. In a directory of `ba-*` plus `ba_pc_align-*`, that run is `ba-*`, the pre-alignment generation, matching the single-run behavior of comparing initial against final within one `bundle_adjust`. Lists returned with `all_files=True` are sorted for the same reason. Pattern precedence is unchanged — the first pattern with any match still wins.
- **An ambiguous single-file lookup is now logged instead of resolved silently.** When `glob_file()` has to choose between several matches it warns with the pattern, every candidate, the file it used, and the suggestion to pass an ASP output prefix (`--bundle-adjust-prefix ba/ba_pc_align`) to select a run. A lookup with exactly one match, or one made with `all_files=True`, is not a choice and stays quiet.
