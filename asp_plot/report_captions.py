...
EARTH_ALIGNMENT_DESCRIPTION = (
    ...
    "  - Median_beg / Median_end: median of the DEM-vs-ICESat "
    "absolute height residuals before / after alignment.\n"
    "  - NMAD_beg / NMAD_end: normalized median absolute "
    "deviation of the residuals before / after alignment; a "
    "robust spread, insensitive to outliers.\n"
    "  - RMSE_beg / RMSE_end: root-mean-square residual before "
    "/ after alignment; outlier-sensitive, so a large RMSE "
    "next to a small NMAD flags a tail of gross errors rather "
    "than a broad misfit.\n"
    "  - p16 / p50 / p84 _beg / _end: shown instead of the "
    "above when the pc_align log predates ASP 3.7.0 (which "
    "added the Median / NMAD / RMSE summary): 16th / 50th / "
    "84th percentile of the absolute residuals before / after "
    "alignment; p50 is the median.\n"
...
def planetary_alignment_description(instrument):
    return (
        ...
        f"  - Median_beg / Median_end: median of the "
        f"DEM-vs-{instrument} absolute height residuals before / "
        f"after alignment.\n"
        f"  - NMAD_beg / NMAD_end: normalized median absolute "
        f"deviation of the residuals before / after alignment; a "
        f"robust spread, insensitive to outliers.\n"
        f"  - RMSE_beg / RMSE_end: root-mean-square residual before "
        f"/ after alignment; outlier-sensitive, so a large RMSE "
        f"next to a small NMAD flags a tail of gross errors rather "
        f"than a broad misfit.\n"
        f"  - p16 / p50 / p84 _beg / _end: shown instead of the "
        f"above when the pc_align log predates ASP 3.7.0 (which "
        f"added the Median / NMAD / RMSE summary): 16th / 50th / "
        f"84th percentile of the absolute residuals before / after "
        f"alignment; p50 is the median.\n"
