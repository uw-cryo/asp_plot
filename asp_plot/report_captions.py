...
GROUND_CONTROL = "Ground control point residuals (DEM minus control height, both above the ellipsoid). Control points are fetched and normalized by the groundcontrol package (survey monuments and checkpoints with rigorous datum/epoch handling), quality-filtered to survey-grade heights, transformed into the DEM frame, and differenced against the bilinearly sampled DEM. Left: residuals over the DEM hillshade. Right: residual distribution with summary statistics."
