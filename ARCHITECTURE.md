...
**`control_points.py`** - Ground-control comparison, **EXPERIMENTAL** (issue #156 prototype)
- Consumes a control-point GeoParquet written by [groundcontrol](https://github.com/uw-cryo/groundcontrol) (`groundcontrol-fetch`): `ControlPoints` quality-filters (NGS survey-grade `vertSource` vintages on usable `posSource` horizontal positions from the `raw` JSON, 3DEP topo-only, NGL dropped — antenna-reference heights), lands the points in the DEM's 3D frame, samples the DEM (reusing `AltimetrySource`'s open/interp statics), and computes DEM − control residual stats; `ControlPointsPlotter(Plotter)` renders the two-panel report figure (residuals over hillshade + histogram)
- Dependency direction is the point of the prototype: **asp_plot optionally imports groundcontrol** at exactly one call site (`_transform_points`, lazy) for the 3D CRS/datum/epoch transform — mixed vertical datums are transformed per uniform-`vertical_crs` group because groundcontrol fail-louds rather than guessing. Points already in the DEM CRS never touch groundcontrol (which is how tests avoid the dependency); anything else without it installed raises an actionable ImportError
- Outlier filter is NMAD-around-median, deliberately **not** the altimetry sources' std filter: with a single blunder among n points the largest |z| is (n−1)/√n, so a 3σ std filter can never reject anything at n ≤ 10 — exactly the sparse-control regime
- Wired into the report as the `ground_control` spec (`--control_parquet` + `--control_epoch`; epoch defaults to the decimal year of the first acquisition date); failures warn and skip the section rather than aborting the report
...
- `--control_parquet`: EXPERIMENTAL (issue #156) — ground-control GeoParquet from `groundcontrol-fetch`; adds a "Ground Control" residual page (see `control_points.py`). `--control_epoch` overrides the DEM decimal-year epoch used by the frame transform
