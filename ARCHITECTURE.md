...
- Layout: `base.py` holds the `SensorMetadata` ABC and shared helpers (`_common_base`, the generic XML lister, the scene-dict defaults); each sensor family gets its own module (`worldview.py`, `dimap.py`); `__init__.py` holds the `SENSORS` registry, the detection entry points, and re-exports every public name, so `from asp_plot.sensors import ...` is unchanged from the pre-split module
- **Detection is content-based** (issue #162): each reader implements a single abstract `_is_camera_file()` content predicate (cheap `iterparse`, stops at the first identifying tags), and `base.py` builds discovery, filtering, `detect`, and `detect_files` on top of it once. A reader never claims another sensor's files just because they end in `.xml`; unrecognized inputs produce the clean "no supported sensor found" error. The WorldView check requires the `<isd>` root **and** the `IMD`/`EPH`/`ATT` blocks — the root alone is not sufficient because ASP's `gen_aster` camera XMLs share it. The DIMAP check requires `METADATA_SUBPROFILE == PRODUCT` and a `METADATA_PROFILE` in `SUPPORTED_DIMAP_PROFILES` (`PHR_SENSOR`, `PNEO_SENSOR`); products from other DIMAP profiles (SPOT 6/7, PeruSat-1) are skipped with a one-time warning naming the profile
