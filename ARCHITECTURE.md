...
- Layout: `base.py` holds the `SensorMetadata` ABC and shared helpers (`_common_base`); each sensor family gets its own module (`worldview.py`, `dimap.py`); `__init__.py` holds the `SENSORS` registry, the detection entry points, and re-exports every public name, so `from asp_plot.sensors import ...` is unchanged from the pre-split module
- `SensorMetadata` ABC (`base.py`) defining the reader interface (`detect` + `detect_files` + `get_scene_dicts`) and the sensor-agnostic scene-dict schema, mirroring the `bodies.py` registry pattern so adding a sensor is a new subclass in a new module with no change to the geometry code
