...
- `notebooks/Pleiades/` - Airbus Pléiades Neo tri-stereo example over Marseille (Earth-based): DIMAP stereo-geometry analysis, fore–aft pair stereo with `--left/right-image-crop-win` (crop windows derived from the DIMAP `Dataset_Extent` vertices), a 3-image multi-view run (nadir reference; the ASP docs discourage MVS in favor of pairwise + `dem_mosaic`, which the notebook cites), mapprojected variants of both chains (COP30 reference converted geoid→ellipsoid via `gdalwarp` vertical datums), and a four-chain DEM comparison
